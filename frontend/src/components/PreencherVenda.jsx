...
const PreencherVenda = ({ anuncio, handleSubmit, handleDelete }) => {  
	// Inicializando estados com dados existentes ou valores padrão
	...
	const handleFormSubmit = (e) => {
		e.preventDefault();
		// Diferenciando entre criação e edição de anúncio
		handleSubmit({ titulo, descricao, valor, images });
	};

	// Preencher os campos ao carregar anúncio para edição
	useEffect(() => {
		if (anuncio) {
	...
	return (
		<div>
			<form className="mt-10 max-w-lg mx-auto" onSubmit={handleFormSubmit}>
				...
				{/* Botão de Remover (apenas se for um anúncio existente) */}
				{anuncio && (
					<button type="button" onClick={() => handleDelete(anuncio.id)} className="mb-10 mt-3 text-white bg-red-500 hover:bg-red-700 focus:ring-4 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5">
						Remover Anúncio
					</button>
				)}
			</form>
